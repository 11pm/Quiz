var data = {
	'Geography': 
	[
		{	
			question: 'What is the capital of Croatia',
			options: [
				{name: 'Zagreb', correct: true},
				{name: 'MURICA', correct: false},
				{name: 'Athens', correct: false},
				{name: 'Sarajevo', correct: false}
			]
		}
	],
	'Tech':
	[
		{
			question: 'What is linux\'s mascots name',
			options: [
				{name: 'Linux', correct: false},
				{name: 'Penguin', correct: false},
				{name: 'Tux', correct: true},
				{name: 'Panda', correct: false}
			]
		},
		{
			question: 'What does JSON stand for',
			options: [
				{name: 'JASON', correct: false},
				{name: 'Javascript Object Notation', correct: true},
				{name: 'Javascript sucks on IE6', correct: false},
				{name: 'Mom\'s spagetti', correct: false}
			]
		},
		{
			question: 'What is the current version of Java',
			options: [
				{name: 'Java SE8', correct: true},
				{name: 'Java 2014', correct: false},
				{name: 'Java 2013', correct: false},
				{name: 'Java version 2014', correct: false}
			]
		}
	],
	'Music':
	[
		{
			question: 'What year did Tupac die',
			options: [
				{name: '1972', correct: false},
				{name: '2002', correct: false},
				{name: '1996', correct: true},
				{name: '2007', correct: false}
			]
		},
		{
			question: 'What member was not in Wu-tang clan',
			options: [
				{name: 'Raekwon', correct: false},
				{name: 'Biggie Smalls', correct: true},
				{name: 'Ghostface Killah', correct: false},
				{name: 'Ol\' dirty bastard', correct: false}

			]
		},
		{
			question: 'From what city is Onyx from',
			options: [
				{name: 'Detroit', correct: false},
				{name: 'Los Angeles', correct: false},
				{name: 'New York', correct: true},
				{name: 'Baltimore', correct: false}
			]
		}
	],
	'Other':
	[
		{
			question: 'What does GUS not teach',
			options: [
				{name: 'GSF2A', correct:true}
			]
		}
	]

};	